...
export const Popup = (Props: PopupProps) => {
    ...
    const changeName = (event: ChangeEvent<HTMLInputElement>) => dispatch({type: 'edit-service', editedPopup: {boxId, isHidden, name: event.currentTarget.value, port}});
    const changePort = (event: ChangeEvent<HTMLInputElement>) => dispatch({type: 'edit-service', editedPopup: {boxId, isHidden, name, port: parseInt(event.currentTarget.value)}});

    const addService = (event: MouseEvent) => {
        event.stopPropagation();
        dispatch({type: 'submit-service', service: {
            name,
            port
        }})
    }
